...
- `ValueError` - ошибка валидации типов/синтаксиса значения
- `FileNotFoundError` - нет файлов данных
- `PermissionError` - запрет операции (например, обновление `__ID`)
...
Повторяющиеся `select` с теми же параметрами могут выполняться быстрее: результат кэшируется через замыкание (`create_cacher`).
После любых операций изменения (`create_table/insert/update/delete/drop_table`) кэш помечается «грязным» и пересчитывается заново при следующем запросе.

## Структура проекта

- `src/primitive_db/engine.py` - CLI и разбор команд
- `src/primitive_db/core.py` - CRUD-логика и декораторы
